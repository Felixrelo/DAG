...
const steps = [
  { icon: ClipboardList, title: "Online-Besichtigung", description: "Füllen Sie unseren Funnel aus und erhalten Sie direkt Ihren ersten Preis." },
  { icon: Users, title: "Beratung & Finalisierung", description: "Unsere Umzugsexperten besprechen alle Details und finalisieren Ihr Angebot." },
  { icon: Truck, title: "Umzugstag & Fertig", description: "Wir kümmern uns um alles – Sie kommen stressfrei Dahoam an." },
];

// Numbered circles variant (default)
function ProcessNumbered({ primaryColor }: { primaryColor: string }) {
  return (
...
// Timeline variant
function ProcessTimeline({ primaryColor }: { primaryColor: string }) {
  return (
...
// Cards variant
function ProcessCards({ primaryColor }: { primaryColor: string }) {
  return (
    ...
    <section id="process" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600">In drei einfachen Schritten zu Ihrem stressfreien Umzug.</p>
        </div>

        {processVariant === "numbered" && <ProcessNumbered primaryColor={primaryColor} />}
        {processVariant === "timeline" && <ProcessTimeline primaryColor={primaryColor} />}
        {processVariant === "cards" && <ProcessCards primaryColor={primaryColor} />}
      </div>
    </section>
  );
}
