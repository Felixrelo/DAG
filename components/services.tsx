...
function ServicesIconCards({ services, primaryColor, t }: { services: string[]; primaryColor: string; t: any }) {
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
      {services.map((service: string) => {
        ...
        const serviceT = t.services[service];
        return (
          <Link key={service} href={href} className="group bg-white rounded-xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">{serviceT?.name || service}</h3>
            <p className="text-gray-600 mb-4">{serviceT?.description || `Professioneller ${service}-Service.`}</p>
...
// Image Cards variant
function ServicesImageCards({ services, primaryColor, serviceImages, t }: { services: string[]; primaryColor: string; serviceImages: Record<string, string>; t: any }) {
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
      {services.map((service: string) => {
        const Icon = serviceIcons[service] || Truck;
          ...
          <Link key={service} href={href} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100">
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-xl font-semibold text-gray-900">{serviceT?.name || service}</h3>
              </div>
              <p className="text-gray-600 mb-4">{serviceT?.description || `Professioneller ${service}-Service.`}</p>
              <div className="flex items-center font-medium" style={{ color: primaryColor }}>
                {t.services.learnMore} <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
  ...
  return (
    <section id="services" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {servicesVariant === "icon-cards" && <ServicesIconCards services={services} primaryColor={primaryColor} t={t} />}
        {servicesVariant === "image-cards" && <ServicesImageCards services={services} primaryColor={primaryColor} serviceImages={serviceImages} t={t} />}
        ...
        <div className="text-center mt-12">
          ...
          >
            {t.cta.getQuote} <ArrowRight className="ml-2 h-5 w-5" />
          </a>
        </div>
      </div>
    </section>
